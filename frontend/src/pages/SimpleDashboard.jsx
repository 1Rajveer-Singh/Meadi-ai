...
import { 
  ...
  TrendingUp, FileText, Stethoscope, Heart, Microscope, Pill,
  ArrowRight, Calendar, Eye, BarChart3, Zap, Sparkles,
  Plus, Filter, Download, RefreshCw, AlertCircle, Target, Layers, Shield,
  Upload, Search, Bell, Settings, Database, Image, 
  PlayCircle, PauseCircle, Gauge, Timer, Award, Cpu,
  Server, Wifi, WifiOff, CheckSquare, XSquare, 
  TrendingDown, AlertOctagon, Info, UserCheck
} from 'lucide-react';
...
const HospitalDashboard = () => {
  ...
  const activityFeed = [
    ...
    },
    {
      ...
      message: "Critical interaction detected: Warfarin + Aspirin for Patient #P-2024-1154",
      severity: "high",
      time: "5 minutes ago", 
      status: "warning",
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600">
                Real-time AI diagnostic monitoring - {format(new Date(), 'EEEE, MMMM d, yyyy')}
              </p>
            </div>
            <div className="mt-4 lg:mt-0 flex items-center space-x-3">
              <div className="flex items-center space-x-2 text-sm">
                <div className={`w-3 h-3 rounded-full ${realTimeUpdates ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`}></div>
                <span className="text-gray-700">{realTimeUpdates ? 'Live Updates' : 'Paused'}</span>
              </div>
              <button
                onClick={() => setRealTimeUpdates(!realTimeUpdates)}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2"
              >
                {realTimeUpdates ? <PauseCircle className="w-4 h-4" /> : <PlayCircle className="w-4 h-4" />}
          ...
          {todayStats.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-l-blue-500 hover:shadow-md transition-shadow"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-600">{stat.title}</p>
                    <p className="text-3xl font-bold text-gray-900 mt-2">
                      <CountUp end={typeof stat.value === 'string' ? 1.8 : stat.value} suffix={typeof stat.value === 'string' ? 'm' : ''} />
                    </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      ...
                      {activity.status === 'success' ? <CheckCircle className="w-5 h-5" /> :
                       activity.status === 'warning' ? <AlertTriangle className="w-5 h-5" /> :
                       activity.status === 'processing' ? <Cpu className="w-5 h-5" /> :
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
              ...
              <p className="text-sm text-gray-600 mt-1">AI diagnosis accuracy and case volume trends</p>
            </div>
            <div className="p-6">
              <div className="h-64">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-pink-50">
              ...
              <p className="text-sm text-gray-600 mt-1">Active cases by medical department</p>
            </div>
            <div className="p-6">
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-green-50 to-blue-50">
              ...
              <p className="text-sm text-gray-600 mt-1">AI processing times by case type</p>
            </div>
            <div className="p-6">
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart 
                    data={[
                      { type: 'X-Ray', avgTime: 1.2, cases: 245, color: '#3B82F6' },
                      { type: 'MRI', avgTime: 3.8, cases: 89, color: '#10B981' },
                      { type: 'CT Scan', avgTime: 2.1, cases: 156, color: '#F59E0B' },
                      { type: 'Ultrasound', avgTime: 0.9, cases: 198, color: '#8B5CF6' },
                      { type: 'ECG', avgTime: 0.5, cases: 312, color: '#EF4444' }
                    ]}
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                    <XAxis 
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {recentDiagnoses.map((diagnosis, index) => (
                    <tr key={diagnosis.patientId} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{diagnosis.diagnosis}</div>
                        <div className="text-sm text-gray-500">{diagnosis.date}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="text-sm font-medium text-gray-900">{diagnosis.confidence}%</div>
                          <div className="ml-2 w-16 bg-gray-200 rounded-full h-2">
                            <div 
                              className={`h-2 rounded-full ${
                                diagnosis.confidence >= 95 ? 'bg-green-500' :
                                diagnosis.confidence >= 90 ? 'bg-blue-500' :
                                'bg-orange-500'
                              }`}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          ...
                          diagnosis.status === 'alert' ? 'bg-red-100 text-red-800' :
                  ...
                  <div key={agent.name} className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{agent.processingSpeed}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{agent.casesToday}</p>
                      </div>
                      <div>
