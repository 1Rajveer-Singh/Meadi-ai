...
const SettingsPage = () => {
  ...
  const fetchAlertRules = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/notifications/alerts`);
      if (response.data.success) {
  ...
  const toggleAlertRule = async (ruleId, enabled) => {
    try {
      await axios.put(`${API_BASE_URL}/api/notifications/alert-rules/${ruleId}`, {
        enabled,
      });
      fetchAlertRules();
        ...
        {hasChanges && (
          <div className="mt-6 pt-4 border-t border-gray-200">
            <div className="flex items-center justify-end space-x-3">
              ...
              <button
                onClick={() => updateNotificationPreferences(notificationPreferences)}
